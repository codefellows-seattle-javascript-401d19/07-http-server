...
describe('server.js', () => {
  describe('HTTP GET requests', () => {
    ...
    describe('Cowsay: /cowsay', () => {
      test('GET request should respond with a 200 status code, and a body if there is no error', () => {
        const url = 'http://localhost:3000/cowsay?text=I%need%something%good%to%say!';
        return superagent
          .get(url)
          .set({ 'Content-Type': 'text/html' })
          .then(response => {
  ...
  describe('HTTP POST requests', () => {
    test('POST request should respond with a 200 status code and a body if there is no error', () => {
      const url = 'http://localhost:3000/api/cowsay';
      const bodyToTest = { 'content': 'testing'};
      return superagent
        .post(url)
        ...
        .then(response => {
          expect(response.status).toEqual(200);
          expect(response.request._data).toEqual(bodyToTest);
        });
    });
