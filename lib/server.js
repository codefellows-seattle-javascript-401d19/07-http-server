...
const app = http.createServer((request, response) => {
  ...
  requestParser.parse(request)
    .then(request => {
      ...
      // logger.log(`info`,`${request.body} is the request.body');
      ...
      else if(request.method === `POST` && request.url.pathname === `/api/cowsay` && request.body){
        logger.log(`info`, `${JSON.stringify(request.url.query)} is the stringified url.query`);
        response.writeHead(200, {'Content-Type' : 'application/json'});
        response.write(`{"content" : ${JSON.stringify(request.body)}}`);
        response.end();
