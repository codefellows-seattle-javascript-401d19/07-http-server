...
const app = http.createServer((request, response) => {
  ...
  requestParser.parse(request)
    .then(request => {
      console.log(JSON.stringify(request.url.query), `is the stringified url.query`);
      ...
      if(request.method === `GET` && request.url.pathname === `/cowsay` && JSON.stringify(request.url.query) != `{}`){
        ...
        response.write(
          ...
          <html>
            ...
            <body>
              <h1> cowsay </h1>
              <pre>
                cowsay.say(${JSON.stringify(request.url.query)})
              </pre>
            </body>
          </html>`);
        response.end();
